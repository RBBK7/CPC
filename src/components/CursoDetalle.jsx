...
export default function CursoDetalle({ cursoId, userId, onBack }) {
  ...
  async function load() {
    const [{ data: c }, { data: e }] = await Promise.all([
      supabase.from('vista_cursos').select('*').eq('id', cursoId).single(),
      supabase.from('estudiantes').select('*').eq('curso_id', cursoId).order('agregado_en', { ascending: true })
    ])
    setCurso(c)
    setEstudiantes(e || [])
    setLoading(false)
  ...
  async function agregar() {
    ...
    setModal(false); setNombre(''); setError('')
    setSaving(false); load()
  }

  async function quitar(id, agregadoPor) {
    if (agregadoPor !== userId) {
      alert('Solo quien agregó a este estudiante puede quitarlo.')
      return
    }
    if (!confirm('¿Quitar a este estudiante del curso?')) return
  ...
  const fechaInicio = curso?.fecha_inicio
    ? new Date(curso.fecha_inicio + 'T00:00:00').toLocaleDateString('es-CR', { year: 'numeric', month: 'long', day: 'numeric' })
    : '—'

  return (
    <div>
      <button className={s.backBtn} onClick={onBack}>← {curso?.ministerio || 'Volver'}</button>
      <div className={s.card}>
        <div className={s.header}>
          <div>
            <div className={s.sectionTitle}>{curso?.curso}</div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: '1rem' }}>
          ...
          {curso?.ministerio && <span className={`${s.badge} ${s.badgeGray}`}>{curso.ministerio}</span>}
        </div>
        ...
        {estudiantes.length === 0
          ...
          : estudiantes.map((e, i) => (
            <div className={s.studentItem} key={e.id}>
              ...
              <button className={`${s.btn} ${s.btnDanger}`} onClick={() => quitar(e.id, e.agregado_por)}>
                Quitar
              </button>
            </div>
          ))
